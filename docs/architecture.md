...
- **Manual Stock vs. Order Integrity:** If a Purchase Order is confirmed and generates stock, should a user be allowed to manually modify that stock batch's quantities outside of the PO? (e.g. extra units were sent as a gift but not paid for, or units were damaged in the warehouse). Currently, manual adjustments are allowed on unconsumed stock, but it decouples the physical stock count from the financial PO record.
